...
const OBJECTIVES_DONE_USER_PATH = "/auth/api/objectives/status/done/user";

export type FetchObjectivesDoneUserResult =
  | { kind: "ok"; objectives: Objective[] }
  | { kind: "unauthorized" }
  | { kind: "network_error" }
  | { kind: "http_error"; status: number };

function normalizeDoneObjectiveItem(raw: unknown): Objective | null {
  ...
  // Rota de concluídos: espera-se `DONE`; outros valores raros viram `DONE` salvo estado claramente em aberto.
  const statusObjective: Objective["statusObjective"] =
    rawStatus === "TODO" || rawStatus === "IN_PROGRESS" ? "IN_PROGRESS" : "DONE";
...
function normalizeDoneObjectivesPayload(body: unknown): Objective[] {
  const out: Objective[] = [];
  const pushItems = (arr: unknown[]) => {
    for (const item of arr) {
      const obj = normalizeDoneObjectiveItem(item);
      if (obj) out.push(obj);
    }
  };
...
/**
 * GET `{API_BASE_URL}/auth/api/objectives/status/done/user` — objetivos concluídos.
 * Header `Authorization: Bearer …`.
 */
export async function fetchObjectivesDoneUser(jwtToken: string): Promise<FetchObjectivesDoneUserResult> {
  const url = `${API_BASE_URL.replace(/\/$/, "")}${OBJECTIVES_DONE_USER_PATH}`;
  ...
  try {
    ...
    if (res.ok) {
      ...
      return { kind: "ok", objectives: normalizeDoneObjectivesPayload(data) };
    }
